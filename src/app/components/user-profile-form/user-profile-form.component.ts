...
export class UserProfileFormComponent {
  ...
  onSubmit() {
    if (this.profileForm.valid) {
      const rawDate = this.profileForm.value.dateOfBirth;
      const rawTime = this.profileForm.value.timeOfBirth;
      let formattedDate = '';
      if ((rawDate as any) instanceof Date) {
        ...
        // If it's already a string (common during 'Edit' mode)
        formattedDate = new Date(rawDate as string).toISOString().split('T')[0];
      }

      // 2. Combine with Time
      const birthDateTime = `${formattedDate}T${rawTime}:00`;
      const newProfile = {
